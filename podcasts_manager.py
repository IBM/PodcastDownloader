...
cnx = mysql.connector.connect(user='root', password='dliu', host='127.0.0.1',
                              database='podcast_downloader')
        ...
        if request.method == 'GET':
            ...
            if item:
                ...
                return app.make_response(
                    'The user info is id %s, name %s' % (id, name))
            else:
...
def _getcustomer(name):
    try:
        ...
        query = ("select customerid, name from customer where "
                 "name = '%s'" % name)
        print(query)
        curA.execute(query)
        item = curA.fetchone()
    ...
    try:
        if request.method == 'GET':
            ...
            if item:
                ...
                return app.make_response(
                    'The podcast info is id %s, podname %s, '
                    'url %s' % (podid, podname, podurl))
            else:
                return 'object not found', 404
        elif request.method == 'POST':
            print('in post')
...
def _getpodcast(podname):
    try:
        ...
        query = ("select podid, podname, url from podcast where "
                 "podname = '%s'" % podname)
        print(query)
        curA.execute(query)
        item = curA.fetchone()
...
def _addpodcast(podname, url):
    try:
        ...
        query = ("insert into podcast (podname,url) values "
                 "('%s', '%s')" % (podname, url))
        print(query)
        curA.execute(query)
        cnx.commit()
...
def _putpodcast(podname, url):
    try:
        print('in _putcustomer')
        query = ("update podcast set url='%s' where "
                 "podname=('%s')" % (url, podname))
        print(query)
        curA.execute(query)
        item = curA.fetchone()
        print(item)
    ...
    try:
        if request.method == 'GET':
            ...
            if item:
                ...
                return app.make_response(
                    'subid %s, customerid %s, podid %s' % (subid,
                                                           customerid,
                                                           podid))
            else:
                return 'object not found', 404
            ...
            if item:
                ...
                return app.make_response(
                    'created subscription %s' % (customerid, podid))
            else:
                return 'error happened', 400
        elif request.method == 'DELETE':
            print('in delete')
...
def _getsub(customerid, podid):
    try:
        print('in _getsub')
        query = ("select subid, customerid, podid from subs where "
                 "customerid = '%s' and podid = '%s'" % (customerid, podid))
        curA.execute(query)
...
def _addsub(customerid, podid):
    try:
        ...
        if customer and podcast:
            query = ("insert into subs (customerid, podid) values "
                     "('%s', '%s')" % (customerid, podid))
            print(query)
            curA.execute(query)
            cnx.commit()
            return True
...
def _putsub(customerid, podid):
    try:
        print('in _putsub')
        query = ("update subs set podid='%s' where "
                 "customerid=('%s')" % (podid, customerid))
        print(query)
        curA.execute(query)
        item = curA.fetchone()
        print(item)
